...
scan_log, trade_log, exit_orders = [], [], []  # each exit has only TP (no SL)
...
def _effective_tp_pct():
    # ensure TP clears round-trip fees + buffer so net win is positive
...
def _place_oco_children(pair, parent_side, qty, entry, tp_price, sl_price):
    ...
    return tp_id, None, tp_res, None

def monitor_exits(prices):
    """
    TP-only monitor + time stop:
    - If TP fills: record P&L and remove exit record.
    - If MAX_HOLD_SEC reached: cancel TP (if any) and flatten at market.
    """
    ...
    for ex in exit_orders:
        pair = ex["pair"]
        side = ex["side"]          # parent side (BUY or SELL)
        qty = ex["qty"]
        tp_id = ex.get("tp_id")
        start_ts = ex.get("start_ts", now_ts)

        # TIME STOP -> cancel TP and flatten at market
        if now_ts - start_ts >= MAX_HOLD_SEC:
            if tp_id:
                try: cancel_order(order_id=tp_id)
                except: pass
        ...
        # Check TP status only
        if tp_id:
            st_tp = get_order_status(order_id=tp_id)
            rem = None
            try:
                rem = float(st_tp.get("remaining_quantity", st_tp.get("remaining_qty", st_tp.get("leaves_qty", 0)) or 0))
            except:
                rem = None
            ...
            if tp_filled:
                ...
                scan_log.append(f"{ist_now()} | {pair} | TP filled")
                to_remove.append(ex)
                pair_cooldown_until[pair] = now_ts + TRADE_COOLDOWN_SEC
                continue
            ...
            if last_candle:
                if int(time.time()) < pair_cooldown_until.get(pair, 0) or _has_open_exit_for(pair):
                    ...
                    if signal:
                        ...
                        if signal["side"] == "BUY":
                            sl_virtual = round(entry - risk_unit, 6)   # sizing only
                            tp = tp_up
                            risk_per_unit = max(entry - sl_virtual, 1e-9)
                        ...
                        if qty <= 0:
                            ...
                            # 2) Confirm entry fill -> record inventory & place ONLY TP
                            parent_id = _extract_order_id(res)
                            if parent_id:
                                st = _wait_for_fill(parent_id)
                                _record_fill_from_status(pair, signal["side"], st, parent_id)

                                tp_id, _, tp_res, _ = _place_oco_children(
                                    pair, signal["side"], qty, entry, tp, sl_virtual
                                )
                                scan_log.append(f"{ist_now()} | {pair} | TP placed | {tp}@{tp_id}")

                                # Save TP bundle for monitoring (NO SL)
                                exit_orders.append({
                                    "pair": pair, "side": signal["side"], "qty": qty,
                                    "tp": tp, "sl": sl_virtual, "entry": entry,
                                    "tp_id": tp_id, "sl_id": None,
