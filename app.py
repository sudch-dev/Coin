...
# ---------- Optional raw I/O (for /io panel) ----------
io_log = deque(maxlen=300)
def io(line: str):
    try:
        io_log.append(f"{ist_now()} | {line}")
    except Exception:
        pass
...
def get_status():
    ...
    # keep coins compact; if rules not loaded yet, return empty map
    coins = {}
    if ALL_USDT_PAIRS:
        # show balances for coins we might trade (USDT markets)
    ...
    return jsonify({
        ...
        "positions": positions or {},        # <-- never None
        "coins": coins or {},                 # <-- never None
        "trades": trade_log[-20:][::-1] if trade_log else [],   # <-- never None
        "scans":  scan_log[-100:][::-1] if scan_log else [],    # <-- never None
    })
