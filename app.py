...
# active quotes {pair: {"bid": None|{id,px,qty,ts}, "ask": ...}}
...
def normalize_qty_for_side(pair, side, price, qty, usdt_avail, coin_avail):
    """
    - Enforce min_qty and qty_precision
    - Enforce min_notional if set
    - Enforce wallet limits per side
    Returns qty (>= min_qty and precision-safe) or 0.0 to skip.
    """
    q = fmt_qty(pair, qty)
    mq = _min_qty(pair)

    # ensure >= min_qty (after precision)
    if q < mq:
        q = fmt_qty(pair, mq)

    # wallet caps
    if side.upper() == "BUY":
        max_buyable = usdt_avail / max(price, 1e-9)
        if q > max_buyable:
...
# -------------------- Exchange calls (with precision/min qty enforcement) --------------------
def place_order(pair, side, qty, price_hint=None, balances=None):
    ...
    q = normalize_qty_for_side(pair, side, price if price > 0 else 1.0, qty, usdt_avail, coin_avail)
    if q <= 0:
        scan_log.append(f"{ist_now()} | {pair} | SKIP market {side} — cannot meet min_qty/notional with wallets")
        return {}
    payload = {
        "market": pair,
        "side": side.lower(),
    ...
    if ("min" in msg and "quantity" in msg) or ("min" in msg and "qty" in msg):
        q_retry = normalize_qty_for_side(pair, side, price if price > 0 else 1.0, max(q, _min_qty(pair)), usdt_avail, coin_avail)
        if q_retry > q:
            payload["total_quantity"] = f"{q_retry}"
            res = _signed_post(f"{BASE_URL}/exchange/v1/orders/create", payload) or {}
            scan_log.append(f"{ist_now()} | {pair} | RETRY market {side} with qty={q_retry} due to min qty error")
    return res
...
def place_limit_order(pair, side, qty, price, balances=None):
    ...
    usdt_avail = (balances or {}).get("USDT", 0.0)
    coin_avail = (balances or {}).get(pair[:-4], 0.0)
    q = normalize_qty_for_side(pair, side, p, qty, usdt_avail, coin_avail)
    if q <= 0:
        scan_log.append(f"{ist_now()} | {pair} | SKIP limit {side} {p} — cannot meet min_qty/notional with wallets")
        return {}
    payload = {
        "market": pair,
        "side": side.lower(),
    ...
    msg = (res.get("message") or "").lower() if isinstance(res, dict) else ""
    if ("min" in msg and "quantity" in msg) or ("min" in msg and "qty" in msg):
        q_retry = normalize_qty_for_side(pair, side, p, max(q, _min_qty(pair)), usdt_avail, coin_avail)
        if q_retry > q:
            payload["total_quantity"] = f"{q_retry}"
            res = _signed_post(f"{BASE_URL}/exchange/v1/orders/create", payload) or {}
            scan_log.append(f"{ist_now()} | {pair} | RETRY limit {side} {p} with qty={q_retry} due to min qty error")
    return res
...
def _boot_kick():
    try:
        load_profit_state()
        fetch_pair_precisions()   # ensure rules exist before first quote
        time.sleep(1.0)
        _start_loop_once()
    except Exception as e:
