...
# Trade management
BE_TRIGGER_PCT = 0.0010             # +0.10% in favor -> move SL to breakeven (we still keep local trailing as a backup)
...
scan_log, trade_log, exit_orders = [], [], []  # exit_orders now holds OCO ids
...
def place_order(pair, side, qty):
    payload = {
        ...
        "order_type": "market_order",     # market entry
...
def place_stop_limit_order(pair, side, qty, stop_price, limit_price):
    """
    NOTE: If CoinDCX uses different field names for stop orders, edit:
...
def _place_oco_children(pair, parent_side, qty, entry, tp_price, sl_price):
    """
    Place exchange-side TP (limit) and SL (stop-limit) orders.
    For BUY parent: child side is SELL; for SELL parent: child side is BUY.
    """
    child_side = "SELL" if parent_side == "BUY" else "BUY"

    # choose child prices with a tiny cushion for stop-limit
    if child_side == "SELL":
        sl_limit = round(sl_price * (1 - 0.0005), 6)  # sell stop -> limit slightly below stop
        tp_res = place_limit_order(pair, child_side, qty, tp_price)
        sl_res = place_stop_limit_order(pair, child_side, qty, sl_price, sl_limit)
    else:
        sl_limit = round(sl_price * (1 + 0.0005), 6)  # buy stop -> limit slightly above stop
        tp_res = place_limit_order(pair, child_side, qty, tp_price)
        sl_res = place_stop_limit_order(pair, child_side, qty, sl_price, sl_limit)

    tp_id = _extract_order_id(tp_res)
    sl_id = _extract_order_id(sl_res)
    return tp_id, sl_id, tp_res, sl_res

def monitor_exits(prices):
    """
    Now checks exchange-side OCO children.
    If one fills -> cancel sibling, record P&L, cooldown.
    Also enforces a time stop by canceling both & flattening at market.
    """
    ...
    for ex in exit_orders:
        # ex schema:
        # { "pair":..., "side": PARENT_SIDE, "qty":..., "tp":..., "sl":..., "entry":...,
        #   "tp_id":..., "sl_id":..., "start_ts":..., "be_armed": bool }
        pair = ex["pair"]; side = ex["side"]; qty = ex["qty"]
        tp = ex["tp"]; sl = ex["sl"]
        tp_id = ex.get("tp_id"); sl_id = ex.get("sl_id")
        start_ts = ex.get("start_ts", now_ts)

        # TIME STOP
        if now_ts - start_ts >= MAX_HOLD_SEC:
            # cancel both children then flatten at market
            if tp_id: cancel_order(order_id=tp_id)
            if sl_id: cancel_order(order_id=sl_id)
            res = place_order(pair, "SELL" if side == "BUY" else "BUY", qty)
            scan_log.append(f"{ist_now()} | {pair} | TIMEOUT FLAT {('SELL' if side=='BUY' else 'BUY')} {qty} | {res}")
            oid = _extract_order_id(res)
            if oid:
                st = _wait_for_fill(oid)
                _record_fill_from_status(pair, "SELL" if side=="BUY" else "BUY", st, oid)
            to_remove.append(ex)
        ...
        tp_filled = False
        if tp_id:
            st_tp = get_order_status(order_id=tp_id)
            # Filled if remaining qty 0 or status indicates fill
            rem = None
            try: rem = float(st_tp.get("remaining_quantity", st_tp.get("remaining_qty", st_tp.get("leaves_qty", 0)) or 0))
            except: rem = None
            status_txt = (st_tp.get("status") or "").lower()
            tp_filled = (rem == 0) or ("filled" in status_txt and "part" not in status_txt)

            if tp_filled:
                # record fill
                _record_fill_from_status(pair, "SELL" if side=="BUY" else "BUY", st_tp, tp_id)
                # cancel sibling SL
                if sl_id: cancel_order(order_id=sl_id)
                scan_log.append(f"{ist_now()} | {pair} | TP filled; canceled SL")
                to_remove.append(ex)
                pair_cooldown_until[pair] = now_ts + TRADE_COOLDOWN_SEC
                continue
        ...
        sl_filled = False
        if sl_id:
            st_sl = get_order_status(order_id=sl_id)
            rem = None
            try: rem = float(st_sl.get("remaining_quantity", st_sl.get("remaining_qty", st_sl.get("leaves_qty", 0)) or 0))
            except: rem = None
            status_txt = (st_sl.get("status") or "").lower()
            sl_filled = (rem == 0) or ("filled" in status_txt and "part" not in status_txt)

            if sl_filled:
                _record_fill_from_status(pair, "SELL" if side=="BUY" else "BUY", st_sl, sl_id)
                if tp_id: cancel_order(order_id=tp_id)
                scan_log.append(f"{ist_now()} | {pair} | SL filled; canceled TP")
                to_remove.append(ex)
                pair_cooldown_until[pair] = now_ts + TRADE_COOLDOWN_SEC
                continue

    for ex in to_remove:
        try: exit_orders.remove(ex)
        except: pass
                ...
                if int(time.time()) < pair_cooldown_until.get(pair, 0) or _has_open_exit_for(pair):
                    ...
                    if signal:
                        ...
                        if qty <= 0:
                            ...
                            # 2) Confirm entry fill -> record P&L inventory & place OCO exits
                            parent_id = _extract_order_id(res)
                            if parent_id:
                                st = _wait_for_fill(parent_id)
                                _record_fill_from_status(pair, signal["side"], st, parent_id)

                                # Place exchange-side TP & SL
                                if signal["side"] == "BUY":
                                    tp_id, sl_id, tp_res, sl_res = _place_oco_children(
                                        pair, "BUY", qty, entry, tp, sl
                                    )
                                else:
                                    tp_id, sl_id, tp_res, sl_res = _place_oco_children(
                                        pair, "SELL", qty, entry, tp, sl
                                    )

                                scan_log.append(f"{ist_now()} | {pair} | OCO placed | TP:{tp}@{tp_id} SL:{sl}@{sl_id}")
                                # Save exit bundle for monitoring
                                exit_orders.append({
